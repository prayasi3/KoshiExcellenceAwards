...
export default function SponsorCard({ sponsor }) {
  ...
  const content = <><div className="flex min-h-40 items-center justify-center rounded-xl border border-slate-100 bg-slate-50 p-6">{hasLogo ? <FacebookMedia src={sponsor.logo_url} alt={`${sponsor.sponsor_name} logo`} onError={() => setImageFailed(true)} className="max-h-24 max-w-full object-contain" placeholder={<span className="font-heading text-3xl font-bold text-[#0B1F3A]">{initials(sponsor.sponsor_name)}</span>} /> : <span className="font-heading text-3xl font-bold text-[#0B1F3A]">{initials(sponsor.sponsor_name)}</span>}</div><div className="mt-5 flex items-start justify-between gap-3"><div><h3 className="text-xl font-bold text-[#0B1F3A]">{sponsor.sponsor_name}</h3><div className="mt-3"><SponsorTier level={sponsor.sponsor_level} /></div></div>{sponsor.website && <ExternalLink className="mt-1 shrink-0 text-[#C9A84C]" size={19} aria-hidden="true" />}</div></>;
